import { ProductCartI } from "@/interfaces/product/product";
import ImageWithLoading from "../share/ImageWithLoading";
import useFindProductByIdFromFav from "@/hooks/GetProductByIdFromFav";
import { Favorite, HeartOutline } from "@/assets/icons";
import { useDispatch } from "react-redux";
import { AddToFav, deleteProductFromFav } from "@/store/slices/favSlice";
import {
  AddQuantity,
  deleteProductFromCart,
  RemoveQuantity,
} from "@/store/slices/modalSlice";
import { roundToTwoDecimalPlaces } from "@/util/Discount";

const CartProductCard = ({ product }: { product: ProductCartI }) => {
  const productInFav = useFindProductByIdFromFav(product?.id);
  const dispatch = useDispatch();

  const handleAddFav = () => {
    if (product) {
      dispatch(AddToFav(product.products));
    }
  };
  const handleRemoveFav = () => {
    if (product) {
      dispatch(deleteProductFromFav(product?.id));
    }
  };

  const handleAddQuantity = () => {
    dispatch(
      AddQuantity({
        id: product?.id,
        quantity: 1,
      })
    );
  };

  const handleRemoveQuantity = () => {
    dispatch(
      RemoveQuantity({
        id: product?.id,
        quantity: 1,
      })
    );
  };

  const handleDeleteFromCart = () => {
    dispatch(deleteProductFromCart(product?.id));
  };

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm md:p-6">
      <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
        <a href="#" className="shrink-0 md:order-1">
          <ImageWithLoading
            className="hidden h-20 w-20 dark:block"
            src={product.products.thumbnail}
            alt={product.products.title}
          />
        </a>
        <label htmlFor="counter-input" className="sr-only">
          Choose quantity:
        </label>
        <div className="flex items-center justify-between md:order-3 md:justify-end">
          <div className="flex items-center">
            <button
              type="button"
              id="decrement-button"
              onClick={handleRemoveQuantity}
              className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-100 "
            >
              <svg
                className="h-2.5 w-2.5 text-gray-900 "
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 18 2"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M1 1h16"
                />
              </svg>
            </button>
            <input
              type="text"
              id="counter-input"
              data-input-counter
              className="w-10 shrink-0 pointer-events-none border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 "
              value={product.quantity}
            />
            <button
              type="button"
              onClick={handleAddQuantity}
              data-input-counter-increment="counter-input"
              className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-100 "
            >
              <svg
                className="h-2.5 w-2.5 text-gray-900 "
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 18 18"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M9 1v16M1 9h16"
                />
              </svg>
            </button>
          </div>
          <div className="text-end md:order-4 md:w-32">
            <p className="text-base font-bold text-gray-900 ">
              ${" "}
              {roundToTwoDecimalPlaces(
                product.products.price * product.quantity
              )}
            </p>
          </div>
        </div>
        <div className="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
          <a
            href="#"
            className="text-base font-medium text-gray-900 hover:underline "
          >
            {product.products.title}
          </a>
          <div className="flex items-center gap-4">
            {productInFav ? (
              <button
                onClick={handleRemoveFav}
                type="button"
                className="flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-gray-900 hover:underline"
              >
                <span className="text-2xl text-red-500">
                  <Favorite />
                </span>
                Remove to Favorites
              </button>
            ) : (
              <button
                onClick={handleAddFav}
                type="button"
                className="flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-gray-900 hover:underline"
              >
                <span className="text-2xl text-red-500">
                  <HeartOutline />
                </span>
                Add to Favorites
              </button>
            )}

            <button
              type="button"
              onClick={handleDeleteFromCart}
              className="inline-flex items-center text-sm font-medium text-red-600 hover:underline dark:text-red-500"
            >
              <svg
                className="me-1.5 h-5 w-5"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                width={24}
                height={24}
                fill="none"
                viewBox="0 0 24 24"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M6 18 17.94 6M18 18 6.06 6"
                />
              </svg>
              Remove
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default CartProductCard;
